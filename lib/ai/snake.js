  ...
  */
  getMove: function(mySnake, world) {
    if (!this.fsm.getCurrentState()) {
      console.log('No start state, using idle');
      this.fsm.push("idle");//this.fsm.push(this.idleState);
    }
    ...
    while (!resp && count > 0) {
      count--;
      //console.log('itt resp: '+resp);
      resp = this.fsm.update(this);
    }
    if (!resp) {
  ...
  idleState: function(mySnake, world) {
    ...
    if (this.foodIsOnTheMap(world)) {
      if ( this.mySnake.health < config.ai.state.idle.needFoodAtHealthLessThan || this.mySnake.coords.length < config.ai.state.idle.needFoodWhenBodyLessThan ) {
        // need food because hungry or too short
        //this.fsm.push('findFoodToEat');
        console.log('idleState - push: need food');
        this.fsm.push('findFoodToEat');
        return;

      } else {
        // head to safe spot
        console.log('idleState - push: head to safe spot');
        this.fsm.push('moveToSafeSpot');
        return;
      }
    } else {
      // no food on the map
      console.log('idleState - push: head to safe spot, no food on map');
      this.fsm.push('moveToSafeSpot');
      return;
    }
  },
  ...
  moveToSafeSpotState: function(mySnake, world) {
    if ( this.mySnake.health < config.ai.state.goingToSafeSpot.needFoodAtHealthLessThan ) {
      console.log('moveToSafeSpotState() low health, popping');
      this.fsm.pop();
      return;
    }
    ...
    if (path && path.length > 0) {
      ...
      var dirString = this.getDirString( this.getDirToLoc(path[0]) );
      return {
        move: dirString,
        taunt: config.snake.taunt.wander
      };
    }
    else 
      return {
  ...
  findFoodToEatState: function(mySnake, world) {
    ...
    var closestFoodCoord = this.findClosestFood(world);

    // pathfind
    var path = this.pathfind([this.head()[0],this.head()[1]], [closestFoodCoord[0],closestFoodCoord[1]]);
    ...
    if (path && path.length > 0) {
      ...
      return {
        move: dirString,
        taunt: config.snake.taunt.goEat
      };
    }
    else {
      var dir = this.getWiseDirection(); // go randomly
      return {
        move: this.getDirString(dir),
        taunt: config.snake.taunt.goEat
      };
...
// -------------------------------------------------------------
  ...
  */
  ...
  findClosestFood: function(world) {
    var closestFoodCoord;
    var closestDist = 10000000;
    for (var i=0; i<world.food.length; i++) {
      if (!closestFoodCoord) {
        closestFoodCoord = world.food[i];
        closestDist = this.d( closestFoodCoord, this.head() );
        console.log('starting with closest food: '+closestFoodCoord+' d: '+closestDist);
      } else {
        var d = this.d( world.food[i], this.head() );
        if (d < closestDist) {
          closestDist = d;
          closestFoodCoord = world.food[i];
          console.log('new closest food: '+closestFoodCoord+' d: '+closestDist);
        }
      }
    }
...
function Board(world) {
  ...
  if (world.board && world.board.width)
    this.width = world.board.width;
  else
    this.width = world.width;
  if (world.board && world.board.height)
